...
const TITLE = ['EXCHANGE HOUSE', 'CASA DE CANJES', '交换屋']
...
const itemList = [ 
    ...
    item.tm.tm144,
    item.tm.tm145,
    item.consumable.scytherEgg,
    item.consumable.pinsirEgg,
    item.consumable.laprasEgg,
    item.consumable.porygonEgg,
    item.consumable.dratiniEgg,
    item.consumable.transmutationFlask,
    item.consumable.shinyFlask,
]

export class ExchangeHouse extends Window {
    constructor(main) {
        super(null, 600, 450);
        this.main = main;
        this.render();
        this.itemInfo = new ExchangeHouseItem(this);
    }
    ...
    listItems() {
        ...
        itemList.forEach((item, index) => {
            ...
            this.martItem[index].name.style.width = '66%';
            this.martItem[index].price.style.width = '22%';
            this.martItem[index].name.innerText = item.name[lang];
            this.martItem[index].price.innerText = (index == 0) ? `$${item.price}` : `${item.price} ${text.pokeMart.tokensShort[lang]}`;

            this.martItem[index].addEventListener('click', () => { this.itemInfo.open(item, index) });
    ...
    update() {
        const lang = this.main.lang;
        this.main.goldUI.innerText = `$${this.main.player.gold}`; 
...
class ExchangeHouseItem extends Popup {
    ...
    amountUpdate() {
        ...
        if (this.indx == 0) {
            this.price.innerText = `$${this.cost}`;
            ...
            this.price.innerText = `${this.cost} ${text.pokeMart.tokens[lang]}`;
            if (this.component.main.inventory.searchItem('token') == undefined || this.cost > this.component.main.inventory.searchItem('token').amount) {
                this.price.style.color = '#C03B3B';
                this.buttonBuy.style.filter = 'brightness(0.8)';
                this.buttonBuy.style.pointerEvents = 'none';
            } else {
    ...
    purchase() {
        (this.indx == 0) ? this.purchaseWithGold() : this.purchaseWithToken();
    }
    ...
    purchaseWithGold() {
        ...
        if (!this.component.main.player.achievement[34].status) {
            this.component.main.player.achievement[34].progress[0] += Number(this.quantity);
        ...
        if (!this.component.main.player.achievement[36].status) {
            this.component.main.player.achievement[36].progress[0] += Number(this.quantity);
        ...
        if (!this.component.main.player.achievement[38].status) {
            this.component.main.player.achievement[38].progress[0] += Number(this.quantity);
    ...
    purchaseWithToken() {
        if (this.cost > this.component.main.inventory.searchItem('token').amount) return;
        playSound('purchase','ui');
        this.component.main.inventory.removeItem(this.component.main.inventory.searchItem('token'), this.cost);
        if (this.item.isEgg) {
            for (let i = 0; i < this.quantity; i++) {
                const eggs = {}
                const eggKey = `eggCasino${Date.now()}-${i}`;
                eggs[eggKey] = this.item;
                eggs[eggKey].key = eggKey;
                this.component.main.inventory.addItem(eggs[eggKey])
            }
        }
        else this.component.main.inventory.addItem(this.item, this.quantity)
        this.component.update();
        this.close(true);
    }
